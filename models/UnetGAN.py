...
class Critic(nn.Module):
    def __init__(self, patchsize, in_ch, input_ft, max_ft=265, depth=4, **kwargs):
        ...
        f_k_s_list = [
            (in_ft[i], in_ft[i + 1], k, s)
            for i, k, s in zip(range(len(in_ft) - 1), [4] * depth, [2] * depth)
        ] + [(in_ft[-1], in_ft[-1], 3, 1)]
...
def gradient_penalty(dev, critic, target, fake, lam=10):
    ...
    fake_ = (
        torch.FloatTensor(target.shape[0], 1).to(dev).fill_(1.0).requires_grad_(False)
    )
...
class down(nn.Module):
    def __init__(self, in_ch, out_ch, downmode, dropout, **kwargs):
        ...
        if downmode == "conv":
            self.down = []
            self.down.append(
                nn.Conv2d(in_ch, in_ch, kernel_size=4, stride=2, padding=1, bias=False)
            )
            if "norm" in kwargs:
                assert kwargs["norm"] in _norm_options, "Norm must be in, bn or an"
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.downs = nn.ModuleList(
            [
                down(ch[i], ch[i + 1], downmode, dropout, **kwargs)
                for i in range(len(ch) - 1)
            ]
            + [down(ch[-1], ch[-1], downmode, dropout, **kwargs)]
        )
        self.ups = nn.ModuleList(
            [
                up(ch[i + 1] * 2, ch[i], upmode, dropout, **kwargs)
                for i in reversed(range(len(ch) - 1))
            ]
            + [up(ch[1], ch[0], upmode, dropout, **kwargs)]
        )
