...
class down(nn.Module):
    def __init__(self, in_ch, out_ch, downmode, batchnorm, dropout):
        ...
        if downmode == 'conv':
            self.down = []
            if batchnorm:
                self.down.append(nn.Conv2d(in_ch, in_ch, kernel_size=4,
...
class UNet(nn.Module):
    def __init__(self, ch=[64, 128, 256, 512], downmode='sample', upmode='conv', batchnorm=False, dropout=None):
        super(UNet, self).__init__()
        self.ch = ch
        self.inc = inconv(1, ch[0])
        self.downs = nn.ModuleList([down(ch[i], ch[i + 1], downmode, batchnorm, dropout)
                                   for i in range(len(ch) - 1)] + [down(ch[-1], ch[-1], downmode, batchnorm, dropout)])
        self.ups = nn.ModuleList([up(ch[i + 1] * 2, ch[i], upmode, batchnorm, dropout)
                                 for i in reversed(range(len(ch) - 1))] + [up(ch[1], ch[0], upmode, batchnorm, dropout)])
