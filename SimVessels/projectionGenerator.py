        ...
        for d in [12,15,20]: # 10 times Proportion between d0 & d1
            for epsilon in range(4, 10): # differ Proportion between length & diameter
                for niter in range(6, 14):
                    ...
                    update = np.loadtxt(
                        inpath + 'update_d' + str(d0mean) + '_dr' + str(d) + '_epsilon' + str(epsilon) + '_iter' + str(
                            niter) + '.txt')
                    print(' ')
                    print('Load: ' + 'update_d' + str(d0mean) + '_dr' + str(d) + '_epsilon' + str(
                        epsilon) + '_iter' + str(
                        niter) + '.txt')
                    ...
                    sigma = 500
                    v = 1
                    t_start = 600; t_end = 1001; t_step = 1000
                    for t in range(t_start, t_end, t_step):
                        Out_volume_name = outpath + "Lnet_d{}_dr{}_epsilon{}_iter{}_SD{}_v{}_t{}_{}x{}x{}.raw".format(
                                                        d0mean, d, epsilon, niter, sigma, v, t, tissueVolume[2], tissueVolume[1], tissueVolume[0])
                        ...
                        try:
                            updaten_bolus, max_dist = bolus_injection(updaten, v, t, sigma=sigma, interp_coords_factor=0)
                        except:
                            print('bad vessel')
                            break
                        ...
                        img = process_network_fluid(updaten_bolus, dmin, dmax, tVol=tissueVolume, rs=False).astype('float32')

                        img.tofile(Out_volume_name)
                        print("Generating Projections……")
                        # -----------Volumes To CT Projection--------#
                        os.system("VascularProjection.exe %s %s %d %d %d 0.15 0.15 0.15 %d %d %d" % (
                        Out_volume_name, OutPath_CTproj, tissueVolume[2], tissueVolume[1], tissueVolume[0],
                        CTsize[0], CTsize[1], num_projection))
