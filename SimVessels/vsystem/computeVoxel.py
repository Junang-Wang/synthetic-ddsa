...
def process_network_fluid(data, dmin, dmax, tVol):
    #  Find non-NaN min/max values across data leaving a 10% margin in case of
    #  vessel/tissue boundary contact
...
def process_network_fluid_constraint(data, dmin, dmax, tVol, phyconst):
    #  Find non-NaN min/max values across data leaving a 10% margin in case of
    #  vessel/tissue boundary contact
    ...
    for i in range(0, data.shape[1]):
        if not np.isnan(data[0, i]):
            # Discretise data into defined tissue volume
            tempvec = discretisation_phyconst(data[:, i], dmin, dmax, tVol, phyconst)
        else:
            tempvec = data[:, i].reshape(-1, 1)
...
def discretisation_phyconst(p1, dmin, dmax, tVol, phyconst):
    x = np.floor(tVol[0] * (p1[0] - dmin[0]) / (dmax[0] - dmin[0]))
    y = np.floor(tVol[1] * (p1[1] - dmin[1]) / (dmax[1] - dmin[1]))
    z = np.floor(tVol[2] * (p1[2] - dmin[2]) / (dmax[2] - dmin[2]))

    # using physical constraint
    pcx = np.array(np.where(phyconst[:, int(y), int(z)] == 1))
    pcy = np.array(np.where(phyconst[int(x), :, int(z)] == 1))
    pcz = np.array(np.where(phyconst[int(x), int(y), :] == 1))

    if x < np.min(pcx):  # pcx[0, 0]:
        x = np.min(pcx) + np.random.randint(0, 4)
    elif x > np.max(pcx):  # pcx[0, 1]:
        x = np.max(pcx) - np.random.randint(0, 4)
    if y < np.min(pcy):
        y = np.min(pcy) + np.random.randint(0, 4)
    elif y > np.max(pcy):
        y = np.max(pcy) - np.random.randint(0, 4)
    if z < np.min(pcz):
        z = np.min(pcz) + np.random.randint(0, 4)
    elif z > np.max(pcz):
        z = np.max(pcz) - np.random.randint(0, 4)
