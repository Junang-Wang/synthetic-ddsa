...
def generate_points(newNodes=None,points=None, usenan=True):
    ...
    if usenan:
        nanVec = np.empty((len(newNodes),1))
        nanVec[:] = np.NaN
...
def resize_network(data,dmin,dmax,tVol):
    newarray = np.array([])
...
def process_network_fluid(data,dmin,dmax,tVol,rs=True):
    #  Find non-NaN min/max values across data leaving a 10% margin in case of
    #  vessel/tissue boundary contact

    #data 5*n, x,y,z,diam,intensity

    newarray = np.array([])
    img_stack = np.empty([tVol[0], tVol[1], tVol[2]], dtype='float32')
    # resize the vessels voxels to desired size
    if rs:
        for i in range(0,data.shape[1]):
            if not np.isnan(data[0,i]):
                # Discretise data into defined tissue volume
...
def transversal(x,y,z,dx,dy,dz,diam,value,img_stack=None,tVol=(600,600,700),
                resolution=(1,1,1)):
    dx = dx - x
    dy = dy - y
    dz = dz - z
        ...
        if exy < 0:
            if exz < 0:
                ...
                if int(x) >= 0 & int(x) < tVol[0]:
                    img_stack = diamVoxels(x,y,z,0,r,value,img_stack,tVol,res=resolution)
            else:
                z += sz
                exz -= bx
                ezy += by
                if int(z) >= 0 & int(z) < tVol[2]:
                    img_stack = diamVoxels(x,y,z,2,r,value,img_stack,tVol,res=resolution)
        else:
            if ezy < 0:
                z += sz
                exz -= bx
                ...
                if int(y) >= 0 & int(y) < tVol[1]:
                    img_stack = diamVoxels(x,y,z,1,r,value,img_stack,tVol,res=resolution)
...
def discretisation(p1,dmin,dmax,tVol):
    ...
    return np.array([x,y,z,p1[3],p1[4]]).reshape(-1,1) if len(p1) > 4 else np.array([x,y,z,p1[3]]).reshape(-1,1)

def diamVoxels(x,y,z,idx,r,value,img_stack,tVol,res):
    '''
                ...
                for j in range(-r,r+1):
                    if (int(z+j) < t2) & (int(z+j) >= 0):
                        if pow(pow(i*r1,2) + pow(j*r2,2),0.5) <= r:
                            img_stack[int(x-1),int(y+i-1),int(z+j-1)] = value
    elif idx == 1:
        for i in range(-r,r+1):
                ...
                for j in range(-r,r+1):
                    if (int(z+j) < t2) & (int(z+j) >= 0):
                        if pow(pow(i*r0,2) + pow(j*r2,2),0.5) <= r:
                            img_stack[int(x+i-1),int(y-1),int(z+j-1)] = value
    else:
        for i in range(-r,r+1):
            if (int(x+i) < t0) & (int(x+i) >= 0):
                for j in range(-r,r+1):
                    if (int(y+j) < t1) & (int(y+j) >= 0):
                        if pow(pow(i*r0,2) + pow(j*r1,2),0.5) <= r:
                            img_stack[int(x+i-1),int(y+j-1),int(z-1)] = value
