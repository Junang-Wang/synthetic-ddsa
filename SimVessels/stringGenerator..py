        ...
        for niter in range(6, 14):
            for epsilon in range(4, 10):  # differ Proportion between length & diameter
                ...
                for d in [12,15,20]: # ratio betwwen d0 to its subbranch
                    ...
                    print('Creating image ... with %i iterations %i dosize %i d ' %(niter, d0mean, d))
                    ...
                    # print(type(update))
                    np.savetxt(outpath + 'update_d'+str(d0mean)+'_dr'+str(d)+'_epsilon'+str(epsilon)+'_iter'+str(niter)+'.txt', update)
