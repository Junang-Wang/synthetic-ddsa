...
class ProcessSlices:
    ...
    def __init__(self, probs, init_patch, final_patch, augment):
        self.seq = iaa.Sequential(
            [
                ...
                iaa.Sometimes(
                    probs["Affine"],
                    iaa.PiecewiseAffine(scale=(0.0, 0.05), mode="constant"),
                    name="piecewiseaffine",
                ),
                iaa.Sometimes(
                    probs["Multiply"], iaa.Multiply((0.7, 1.3)), name="multiply"
                ),
            ],
            random_order=True,
        )
    ...
    def forward(self, images):
        if self.augment:
            ...
            return [
                im[x : x + self.final_patch, y : y + self.final_patch] for im in images
            ]


class Data(Dataset):
    def __init__(
        self, opt, mode="train", normalization={"type": "global", "mean_std": None}
    ):
        ...
        for subidx, idx in enumerate(np.where(self.info["type"] == self.mode)[0]):
            ...
            stack = np.fromfile(
                os.path.join(self.datafolder, self.info.loc[idx, "data"]),
                dtype="float32",
            ).reshape([i for i in self.info.loc[idx, ["z", "x", "y"]]])
            self.stacks.append(stack)

            mask = np.fromfile(
                os.path.join(self.datafolder, self.info.loc[idx, "mask"]),
                dtype="float32",
            ).reshape([1] + [i for i in self.info.loc[idx, ["x", "y"]]])

            self.masks.append(np.repeat(mask, self.info.loc[idx, "z"], axis=0))
            self.dataset_idx.append(np.repeat(subidx, self.info.loc[idx, "z"], axis=0))
        ...
        if self.normalization["type"] == "global":
            ...
            if self.normalization["mean_std"] is None:
                self.normalization["mean_std"] = (
                    np.mean(self.stacked),
                    np.std(self.stacked),
                )

            self.stacks = [
                (self.stacks[i] - self.normalization["mean_std"][0])
                / self.normalization["mean_std"][1]
                for i in range(len(self.stacks))
            ]
            self.masks = [
                (self.masks[i] - self.normalization["mean_std"][0])
                / self.normalization["mean_std"][1]
        ...
        self.n_slices = np.sum([self.shapes[i][0] for i in range(self.n_datasets)])

        self.stacks = [
            torch.from_numpy(self.stacks[i]).share_memory_()
            for i in range(self.n_datasets)
        ]
        self.masks = [
            torch.from_numpy(self.masks[i]).share_memory_()
            for i in range(self.n_datasets)
        ]
    ...
    def __getitem__(self, idx):
        def _get_sample():
            if self.sample_strategy == "dataset":
                ...
                slice_idx = np.where(
                    np.where(self.dataset_idx == self.dataset_idx[all_slices_idx])[0]
                    == all_slices_idx
                )[0][0]
            ...
            return {
                "x": torch.unsqueeze(torch.from_numpy(input.copy()), 0),
...
def denormalize(stack, start, end, normalization):
    if normalization["type"] == "global":
        ...
        stack = stack * normalization["mean_std"][1][start:end, np.newaxis, np.newaxis]
    return stack
...
def apply_to_raw(
    ...
    stack = np.fromfile(os.path.join(opt.datafolder, name), dtype="float32").reshape(
        [z, x, y]
    )
    ...
    if normalization["type"] == "global":
        ...
        normalization["mean_std"] = np.transpose(
            [
                (np.mean(stack[i, :, :]), np.std(stack[i, :, :]))
                for i in range(len(stack))
            ]
        )
        stack = (
            stack
            * normalization["mean_std"][0][:, np.newaxis, np.newaxis]
            / normalization["mean_std"][1][:, np.newaxis, np.newaxis]
        )
    ...
    for i in range(stack.shape[0]):
        inputs = Variable(
            torch.unsqueeze(torch.unsqueeze(torch.from_numpy(stack[i, :, :]), 0), 0)
        )
        ...
        output = network(inputs)
        output = (
            output[0, 0, :, :].data.cpu().numpy()
            if opt.cuda or opt.m_cuda
            else output[0, 0, :, :].data.numpy()
        )
    ...
    stack_out.tofile(os.path.join(opt.savepath, "%s_%s" % (type, name)))
    log_imgs["applied"].append(
        stack_out[stack_out.shape[0] // 2]
    )  # log the middle frame
    del stack_out
    if (epoch == 0) & (mask != None):
        stack = np.fromfile(
            os.path.join(opt.datafolder, name), dtype="float32"
        ).reshape([z, x, y])
        mask = np.fromfile(os.path.join(opt.datafolder, mask), dtype="float32").reshape(
            [1, x, y]
        )
        masks = mask.repeat(z, axis=0)
        target = stack - masks
        target.tofile(os.path.join(opt.savepath, "%s_gt_%s" % (type, name)))
...
def apply_to_stacks(Dataset, use, network, epoch, opt, normalization):
    ...
    for idx in idxs:
        ...
        log_imgs = apply_to_raw(
            name, type, x, y, z, network, epoch, opt, normalization, log_imgs, mask
        )
...
def normalize_to_min_max(array, min, max):
    ...
    return (
        (((max - min) * (array - min_arr)) / (max_arr - min_arr + 1e-5)) + min,
        min_arr,
        max_arr,
    )
