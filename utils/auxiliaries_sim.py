...
class ProcessSlices:
    ...
    def activator(self, images, augmenter, parents, default):
        return (
            False
            if augmenter.name in ["multiply", "contrastnormalization", "averageblur"]
            else default
        )
...
class Data_CTP_sim(Dataset):
    ...
    def __init__(
        ...
        for p_name in os.listdir(os.path.join("CT_projs", self.datafile)):
            ...
            mask = np.fromfile(
                os.path.join("CT_projs", self.datafile, p_name), dtype="float32"
            ).reshape(int(shape[-1]), int(shape[-2]), int(shape[-3]))
            if self.add_noise:
                mask = NoiseSampling(mask, m_f=1000)
            # mask normalization
            mask = self.M_final - np.array(
                [
                    self.M_final
                    * (mask[i, :, :] - np.min(mask[i, :, :]))
                    / (np.max(mask[i, :, :]) - np.min(mask[i, :, :]))
        ...
        for idx_name in os.listdir(self.vessel_file[0]):
            ...
            target = np.fromfile(
                self.vessel_file[0] + idx_name, dtype="float32"
            ).reshape(int(shape[-1]), int(shape[-2]), int(shape[-3]))
            print(idx_name)
        ...
        self.masks = [
            torch.from_numpy(self.masks[i]).share_memory_() for i in range(self.n_masks)
        ]
        self.targets = [
            torch.from_numpy(self.targets[i]).share_memory_()
    ...
    def __getitem__(self, idx):
        def _get_sample():
            ...
            if self.normalization["type"] == "global":
                input = (
                    input - self.normalization["mean_std"][0]
