...
def main():
    ...
    parser.add_argument('--pretrained_model', default='DeepDSA_2023_04_20__120003_qpds',
    ...
    Data_Train = aux.Data_CTP_sim(opt, mode='train', normalization={
    ...
    Train_mean_std = Data_Train.normalization[
        'mean_std'] if opt.normalization == 'global' or opt.normalization == 'local' else None
    ...
    critic = Critic(opt.final_patch, in_ch=2, input_ft=32, depth=5, max_ft=512, norm=opt.critic_norm)  # input_ft was 64 in earlier experiments
    ...
    for epoch in range(opt.nepochs):
        ...
        train_losses_log = {l: 0.0 for l in ["d_loss", "grad_p_loss", "g_loss_perc", "g_loss_pix", "g_loss_adv", "g_loss"]}
        val_losses_log = {l: 0.0 for l in ["d_loss", "grad_p_loss", "g_loss_perc", "g_loss_pix", "g_loss_adv", "g_loss"]}
        ...
        for i_batch, sample_batched in enumerate(Dataloader_Train):
            ...
            loss_G = opt.beta * Loss_G_pix + loss_G_adv # loss_G = opt.alpha * loss_G_perc + opt.beta * Loss_G_pix + loss_G_adv
            loss_G.backward()
            g_optimizer.step()

            train_losses_log['d_loss'] += loss_D.data.item()
            train_losses_log['grad_p_loss'] += grad_p.item()
            train_losses_log['g_loss_perc'] += 0. #loss_G_perc.item()
        ...
        aux.write_log(os.path.join(opt.savepath, "Log_g.csv"), epoch, stop -
                      start, g_optimizer.param_groups[0]['lr'], train_losses_log['g_loss'],
                      +val_losses_log['g_loss'])
        aux.make_learning_curves_fig(os.path.join(opt.savepath, "Log_g.csv"), att='_g')

        aux.write_log(os.path.join(opt.savepath, "Log_g_pix.csv"), epoch, stop -
                      start, g_optimizer.param_groups[0]['lr'], train_losses_log['g_loss_pix'],
                      val_losses_log['g_loss_pix'])
        aux.make_learning_curves_fig(os.path.join(opt.savepath, "Log_g_pix.csv"), att='_g_pix')

        aux.write_log(os.path.join(opt.savepath, "Log_d.csv"), epoch, stop -
                      start, g_optimizer.param_groups[0]['lr'], train_losses_log['d_loss'],
        ...
        if train_losses_log['d_loss'] > best_train_loss_D:
            ...
            state_train = {'model_G': net.state_dict(), 'model_D': critic.state_dict(), 'normalization': Data_Train.normalization}
        ...
        if train_losses_log['g_loss_pix'] < best_train_loss_G_pix:
            ...
            train_log_imgs = aux.apply_to_stacks(
                Data_Train, [0], net, epoch, opt, Data_Train.normalization)#0,5,6
            state_train = {'model_G': net.state_dict(),'model_D': critic.state_dict(),  'normalization': Data_Train.normalization}
            torch.save(state_train, os.path.join(
                opt.savepath, 'best_train_net_G_pix.pt'))
        ...
        if val_losses_log['g_loss_pix'] < best_val_loss_G_pix:
            ...
            state_train = {'model_G': net.state_dict(), 'model_D': critic.state_dict(), 'normalization': Data_Train.normalization}
            torch.save(state_train, os.path.join(
                opt.savepath, 'best_val_net_G_pix.pt'))
