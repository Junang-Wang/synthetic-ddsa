...
from models.UnetGAN import UNet
...
def main():
    for m in range(len(model_paths)):
        ...
        print(model_path)
       #------------------------   load parameters   ----------------------------#
        Hyperparameters = json.load(open(os.path.join(os.getcwd(), "results", model_path, 'Hyperparameters.txt'), "r"))
        ...
        net = UNet(ch=ch, downmode=opt.downmode,
                   upmode=opt.upmode, dropout=opt.dropout, clahe=opt.clahe)
        if (hasattr(opt, 'm_cuda') and opt.m_cuda) or opt.cuda:
        ...
        #-----------------normalization saved----------------------#
        checkpoint = torch.load(os.path.join(os.getcwd(), "results", model_path, model_name))
        ...
        '''-------------------------    validation    -------------------------'''
        opt.savepath = os.path.join(os.getcwd(), results_folder, model_path) # changed for restoring valuation results
        ...
        for num in range(len(eval_names)):
            ...
            if normalization['type'] == 'local': normalization['type'] = 'global'
            aux.apply_to_raw(eval_name,model_name[5:8], video_size[0], video_size[1], video_size[2], net, 0,
                                         opt, normalization, {'applied': [], 'target': []}, self_norm=self_norm)

        '''--------------------------    Save Logs    ---------------------------'''
        stop = time.time()
        time_elapsed = stop - start
        print('Valuation complete in {:.0f}m {:.0f}s'.format(
