...
def main():
    ...
    parser.add_argument('--use_combine_patches', action='store_true',
                        help='Use pre-saved tmp folder instead.')
    parser.add_argument('--cuda', action='store_true', help='Enables cuda')
    parser.add_argument('--sim_vessel', action='store_true', help= "Use simulated vessels for training")
    ...
    parser.add_argument('--add_noise', action='store_true',
                        help='add poisson noise to CT projections')
    ...
    parser.add_argument('--pretrained_model', default='DeepDSA_2023_02_27__041141',
    ...
    start = time.time()
    probs = {'Blur': opt.pBlur, 'Affine': opt.pAffine,
             'Multiply': opt.pMultiply, 'Contrast': opt.pContrast}

    if opt.save_ram:
        print('use save_ram for train', opt.RamDisk)
        #  normalization values same with original dataset without cropping
        Data_Train = aux.Data_SaveRam(opt.ntrain, opt.datafolder, opt.RamDisk, opt.clahe, probs=probs, mode='train',
                                      normalization={'type': opt.normalization, 'mean_std': (
                                          424.6548767089844, 360.46417236328125), 'clahe_mean_std': (4608.6997, 11309.425), 'min_max': None},
                                      init_patch=opt.init_patch, final_patch=opt.final_patch, augment=opt.augment, sample_strategy=opt.sample_strategy, rejection=opt.rejection)
        ...
        Data_Train = aux.Data_vessel(opt, mode='train', normalization={
            'type': opt.normalization, 'mean_std': None, 'target_mean_std': None, 'clahe_mean_std': None})
    else:
        ...
        opt.vessel_file = ["vessels\\flowing_vessels\\30-60_new\\train_flow\\"]  # flowing vessels 30-60
        # opt.vessel_file = ["vessels\\bolus_chase_t600_train\\", 'vessels\\bolus_chase_ts_train\\']
        Data_Train = aux.Data_CTP_sim_equ_mask(opt, mode='train', normalization={
            'type': opt.normalization, 'mean_std': None, 'min_max': None, 'target_mean_std': None, 'clahe_mean_std': None}) #[432, 223]
    if opt.m_cuda:
        train_sampler = DistributedSampler(Data_Train)
        Dataloader_Train = DataLoader(Data_Train, batch_size=opt.mbs,
                                  num_workers=opt.n_workers, worker_init_fn=aux.worker_init_fn, sampler=train_sampler, pin_memory=True)
    else:
    ...
    Train_mean_std = Data_Train.normalization['mean_std'] if opt.normalization == 'global' or opt.normalization == 'local' else None
    Train_mean_std = Train_mean_std + Data_Train.normalization['clahe_mean_std'] if opt.clahe else Train_mean_std
    opt.mean_std = [
        float(x) for x in Train_mean_std] if opt.normalization == 'global' or opt.normalization == 'local' else None
    Train_min_max = Data_Train.normalization['min_max'] if opt.normalization == 'min_max' or opt.normalization == 'local_min_max' else None
    opt.min_max = [
        float(x) for x in Train_min_max] if opt.normalization == 'min_max' or opt.normalization == 'local_min_max' else None

    opt.vessel_file = ["vessels\\test\\"]
    Data_Val = aux.Data_CTP_sim_equ_mask(opt, mode='val', normalization=Data_Train.normalization)

    if opt.m_cuda:
        val_sampler = DistributedSampler(Data_Val)
        Dataloader_Val = DataLoader(Data_Val, batch_size=opt.mbs, num_workers=opt.n_workers, worker_init_fn=aux.worker_init_fn,
    ...
    net = UNet(ch=ch, downmode=opt.downmode,
               upmode=opt.upmode, batchnorm=opt.batchnorm, dropout=opt.dropout, clahe=opt.clahe)
    if opt.use_trained_model:
        checkpoint = torch.load(
            os.path.join(os.getcwd(), 'results', opt.pretrained_model, "best_train_net.pt"))
    ...
    for epoch in range(opt.nepochs):
        ...
        with torch.no_grad():
            for i_batch, sample_batched in enumerate(Dataloader_Val):
                ...
                if not opt.m_cuda or dist.get_rank() == 0:
                    if i_batch == 0:
                        val_patches = {'input': input[:6, 0, :, :].data.cpu().numpy(),
                                   'target': target[:6, 0, :, :].data.cpu().numpy(),
                                   'output': output[:6, 0, :, :].data.cpu().numpy()}
        ...
        if not opt.m_cuda or dist.get_rank() == 0:
            ...
            logger.add_images('val patch input',  aux.min_max_norm(np.expand_dims(val_patches['input'], 1)), epoch + 1)
            logger.add_images('val patch target', aux.min_max_norm(np.expand_dims(val_patches['target'], 1)), epoch + 1)
            logger.add_images('val patch output', aux.min_max_norm(np.expand_dims(val_patches['output'], 1)), epoch + 1)
